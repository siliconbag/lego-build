...
Фото превращается в мозаику из круглых и квадратных плиток 1×1 на черных пластинах 16×16, как в наборах LEGO Art. Палитра подбирается под лицо из настоящих цветов плиток, выходят картинка, карта сборки с номерами цветов, список деталей и ролик сборки.

```bash
node tools/mosaic.cjs photo.jpg --video
```

Или откройте `mosaic.html`, перетащите фото и крутите настройки прямо в браузере. Фото никуда не отправляется.
