...
|---|---|---|
| `--size` | сторона мозаики в шипах, кратно 16 | 64 |
...
| `--colors` | сколько цветов взять | 12 |
| `--dither` | смешивание соседних плиток, от 0 до 1 | 0.5 |
| `--contrast`, `--sharpen` | растяжка тонов и резкость черт, от 0 до 1 | 0.5 |
| `--zoom`, `--dx`, `--dy` | кадрирование лица | 1, 0, 0 |

Или откройте `mosaic.html`, перетащите фото и крутите те же настройки в браузере. Оттуда же скачиваются картинка, карта сборки и видео. Фото никуда не отправляется.
